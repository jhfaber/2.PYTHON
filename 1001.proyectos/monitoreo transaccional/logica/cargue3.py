...
def fun_valida_caracteres_especiales(indice):
    ...
    if valida_caracteres_especiales==1:
        ...
        #if indice==8:
            #print gldfcompara[gldfcompara.columns[numero]].apply(lambda x: elimina_tildes2(x))
        arr2=gldfcompara[gldfcompara.columns[numero]].apply(lambda x: elimina_tildes(x)).str.match('^([ ,.,a-z,0-9,A-Z,Ñ,ñ]{0,})$')
        if indice==8:
            print gldfcompara.loc[arr2==False,["Segundo Apellido"]]
...
def fun_valida_calculo(indice):
    ...
    if verifica_calculo==1:
        arr3=gldfcompara[gldfcompara.columns[numero]].isnull()
        arrvf=gldfcompara[gldfcompara.columns[int(calculo_verificado[0])-1]].astype('unicode').str.replace(".","",1).str.isdigit()
        arrvf2=gldfcompara[gldfcompara.columns[int(calculo_verificado[1])-1]].astype('unicode').str.replace(".","",1).str.isdigit()
...
def elimina_acentos_columnas(df):
    ...
    while n<len(df.columns):
        lista_col[df.columns[n]]=elimina_tildes2(df.columns[n])
...
while cont_arc<len(gl_archivos):
    ...
    print df.loc[839,["Segundo Apellido"]]
