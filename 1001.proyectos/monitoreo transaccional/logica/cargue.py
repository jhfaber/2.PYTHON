...
def homolga(indice):
    ...
    #if indice==20:
        #print gldfcompara[gldfcompara.columns[numero]]
        #print campo_destino
    if columna.get("entero")==1:
        ...
        arr2=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(".","",1).str.match("^-{0,1}[0-9]{1,15}$")
    ...
    if columna.get("sin_decimal")==1:
        gldfcompara[gldfcompara.columns[numero]]=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(".","",1)
    if tipo==1:
        campo_texto=columna["tabla_texto"]

        if ("tabla_texto2" in columna) == True:
            campo_texto=campo_texto+"+'_'+"+columna["tabla_texto2"] + " as " + columna["tabla_texto"]
           
        consulta="select distinct "+columna["tabla_llave"]+","+campo_texto+" from "+columna["tabla_homologa"] + " tb1 "
        ...
        if columna.get("entero")==1:
            arr2a=dts[0][columna["tabla_texto"]].astype('unicode').str.replace(".","",1).str.match("^-{0,1}[0-9]{1,15}$")
        ...
        if len(dts[0])>0:
            ...
            if ("numero_columna_excel2" in columna) ==True:
                numero2=columna["numero_columna_excel2"]-1
                gldfcompara[gldfcompara.columns[numero2]+"_min"]=gldfcompara[gldfcompara.columns[numero2]].apply(lambda x: elimina_tildes(x).lower().strip().replace(".0","").replace(",",""))
                gldfcompara[gldfcompara.columns[numero]+"_min"]=gldfcompara[gldfcompara.columns[numero2]+"_min"]+"_"+gldfcompara[gldfcompara.columns[numero]+"_min"]
            ...
            #if indice==0:
                #print  gldfcompara.loc[:,gldfcompara.columns[numero]+"_min"]
                #print dts[0].loc[dts[0]["CLIEN_NumeroIdentificacion"]=='41687037',columna["tabla_texto"]+"_min"]
        ...
        #print(gldfcompara.head())
            ...
            #print gldfcompara.iloc[5,:]
            #print gldfcompara.iloc[4]
            #print gldfcompara.iloc[5]
            #print gldfcompara.iloc[3]
            #print columna["tabla_texto"]+"_min"
            #print campo_destino
...
def fun_valida_correo(indice):
    ...
    if es_correo==1:
        ...
        #gldfcompara[gldfcompara.columns[numero]]=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(' 00:00:00','').str.replace('NaT','')
        #print gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(' 00:00:00','').str.lower()
        arr2=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.match('^[(A-Za-z0-9\_\-\.)]+@[(A-Za-z0-9\_\-\.)]+\.[(A-Za-z)]{2,15}$')
        arr2[arr3==True]=True
        rechazar_registro=columna["rechazar_registro"]
        if rechazar_registro==1:
            glregistos_rechazados.loc[arr2==False]=False          
...
def fun_valida_calculo(indice):
    ...
    if len(calculo_verificado)==1:
        calculo_verificado=columna["calculo_verificado"].split('-')
        signo_mas=False
    #print(es_fecha)
    if verifica_calculo==1:
        arr3=gldfcompara[gldfcompara.columns[numero]].isnull()
        arrvf=gldfcompara[gldfcompara.columns[int(calculo_verificado[0])-1]].astype('unicode').str.replace(".","",1).str.match("^-{0,1}[0-9]{1,15}$")
        arrvf2=gldfcompara[gldfcompara.columns[int(calculo_verificado[1])-1]].astype('unicode').str.replace(".","",1).str.match("^-{0,1}[0-9]{1,15}$")
        arrvf2[arrvf==False]=False
        
        #print arrvf2.describe()
        gldfcompara[gldfcompara.columns[numero]+"_compara"]=False
        gldfcompara[gldfcompara.columns[numero]+"_c1"]=0
        gldfcompara[gldfcompara.columns[numero]+"_c2"]=0
        
        #print gldfcompara[gldfcompara.columns[int(calculo_verificado[1])-1]].astype(str).apply(lambda x: float(x))
        gldfcompara.loc[arrvf2,gldfcompara.columns[numero]+"_c1"]=gldfcompara.loc[arrvf2,gldfcompara.columns[int(calculo_verificado[0])-1]].astype(str).apply(lambda x:float(x))
        gldfcompara.loc[arrvf2,gldfcompara.columns[numero]+"_c2"]=gldfcompara.loc[arrvf2,gldfcompara.columns[int(calculo_verificado[1])-1]].astype(str).apply(lambda x: float(x))
        #print gldfcompara[gldfcompara.columns[numero]+"_c2"]
        #print gldfcompara.columns[numero]
        ...
        if signo_mas==False:
            arr2=gldfcompara[gldfcompara.columns[numero]]==gldfcompara[gldfcompara.columns[numero]+"_c1"]-gldfcompara[gldfcompara.columns[numero]+"_c2"]
        else:
            arr2=gldfcompara[gldfcompara.columns[numero]]==gldfcompara[gldfcompara.columns[numero]+"_c1"]+gldfcompara[gldfcompara.columns[numero]+"_c2"]
        ...
        #if indice==32:
            #print gldfcompara.loc[:,gldfcompara.columns[numero]]
            #print gldfcompara.loc[:,[gldfcompara.columns[numero]+"_c1",gldfcompara.columns[numero]+"_c2"]]
            #print arr2
        arr2[arr3==True]=True
        rechazar_registro=columna["rechazar_registro"]
        if rechazar_registro==1:
...
def fun_valida_mayor(indice):
    columna=glvalidacion_asociado["datos_archivo"][indice]
    numero=columna["numero_columna_excel"]-1

    solo_mayores_igual=columna["solo_mayores_igual"]
    if solo_mayores_igual==1:
        campo_destino=columna["campo_destino"]
        mayor_igual=float(columna["mayor_igual"])
        
        arr3=gldfcompara[gldfcompara.columns[numero]].isnull()
        #gldfcompara[gldfcompara.columns[numero]]=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(' 00:00:00','').str.replace('NaT','')
        #print gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(' 00:00:00','').str.lower()
        #arr2=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(".","",1).str.isdigit()
        gldfcompara[gldfcompara.columns[numero]+"_my"]=0
        arr2p=gldfcompara[gldfcompara.columns[numero]].astype('unicode').str.replace(".","",1).str.match("^-{0,1}[0-9]{1,15}$")
        gldfcompara.loc[arr2p,gldfcompara.columns[numero]+"_my"]=gldfcompara.loc[arr2p,gldfcompara.columns[numero]].astype(str).apply(lambda x:float(x))
        #if indice==32:
            #print arr2
        arr2=gldfcompara[gldfcompara.columns[numero]+"_my"]>=mayor_igual
        arr2[arr3==True]=True
        gldfcompara[gldfcompara.columns[numero]].fillna(0,inplace=True)
        glgeneral[campo_destino]=gldfcompara[gldfcompara.columns[numero]]
        glgeneral.loc[arr2==False,campo_destino]=None
        ...
        arr2.iloc[:]=True
    #print(len(arr2))
    agrega_tabla_validacion(indice,columna,arr2,"mensaje_mayor_igual")
    glfun_valida_mayor_igual.append(arr2) 
...
while cont_arc<len(gl_archivos):
    ...
    #print df[df.columns[1]]=='1072254568'
    #print df.loc[df[df.columns[1]]==1072254568,:]
    #print df.iloc[1464,:]
    ...
    #json_asociado = json_asociado.encode('utf8')
    #print json_asociado
    glvalidacion_asociado=ast.literal_eval(json_asociado)
    #s = r'\xe6\x89\x8e\xe5\x8a\xa0\xe6\x8b\x89'
    #glvalidacion_asociado=ast.literal_eval('"%s"'%s).decode('utf-8')
    #print glvalidacion_asociado
    ...
    #print glgeneral["CLXPR_NumeroProducto"]
    caden=caden.replace("'","''")
    #print caden 
...
#3114
#print glgeneral_calidad.iloc[20000]
#print len(glgeneral_calidad)
